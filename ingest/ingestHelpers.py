...
def load_file(filepath):
    """
    Helper function to load the .rq files and return a String object w/
    replacing '\n' by ' '.
...
def sparql_describe(endpoint, query):
    ...
    """
    data = load_settings()
    logging.debug('API settings loaded')
    try:
        EMAIL = data["api_user"]
        PASSWORD = data["api_password"]
        endpoint = data["query_api_url"]
...
def get_metadata(es_index, es_type, es_id):
    ...
    :return:
        a JSON-format string representing the metadata information of the
        object, e.g. {"index": {"_id": "http://...", "_type": "dataset",
        "_index": "dco"}}
    """
...
# get_authors: object -> [authors] for objects such as: datasets,
# publications, ...
def get_authors(ds):
    authors = []
...
# get_distributions: object -> [distributions] for objects such as: datasets,
# publications, ...
